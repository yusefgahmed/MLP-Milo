...
def kcal_per_mole_to_joule(energy_kcal_per_mole):
    """Convert energy from kcal/mol to joules."""
    return energy_kcal_per_mole * sc.KCAL_PER_MOLE_TO_JOULE
...
def sample(ps):
    ...
    for group_index in ps.vibrations:
        ...
        # Add energy to the 10th vibrational mode
        mode_index = 5  # 0-based index for the 6th mode
        additional_energy_kcal_per_mole = 5  # Energy in kcal/mol
        additional_energy_joule = kcal_per_mole_to_joule(additional_energy_kcal_per_mole)
        mode_energies[mode_index] += additional_energy_joule
        ...
        for i, (energy, velocity, has_ZPE) in enumerate(zip(
            ...
            if i == 6:
                added_energy_kcal_per_mole = additional_energy_kcal_per_mole
                print(f"    {i:>4}    {energy * sc.JOULE_TO_KCAL_PER_MOLE:17.6f}"
                      f"   {velocity:14.6e}   {print_ZPE:>12}"
                      f"   (+{added_energy_kcal_per_mole:.2f} kcal/mol)")
            else:
                print(f"    {i:>4}    {energy * sc.JOULE_TO_KCAL_PER_MOLE:17.6f}"
                      f"   {velocity:14.6e}   {print_ZPE:>12}")
